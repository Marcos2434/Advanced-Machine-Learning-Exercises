...
class DDPM(nn.Module):
    ...
    def negative_elbo(self, x):
        ...
        epsilon = torch.randn_like(x) # sample from ε ~ 𝒩(0, I)
        
        # scalars since we have fixed betas
        alpha_t = 1 - self.beta_T
        alpha_bar_t = torch.prod(alpha_t) # equivalent to alpha_t if we keep fixed betas
        
        epsilon_theta = self.network(torch.sqrt(alpha_bar_t) * x + torch.sqrt(1 - alpha_bar_t) * epsilon, t)
