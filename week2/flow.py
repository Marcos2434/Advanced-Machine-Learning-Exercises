...
class MaskedCouplingLayer(nn.Module):
    ...
    def forward(self, z):
        ...
        T = lambda z : b * z + (1 - b) * (z * torch.exp(s(b * z)) + t(b * z))
        # log_det_J = torch.zeros(z.shape[0])
        log_det_J = torch.sum((1 - b) * s(b * z), dim=1)
        
        return T(z), log_det_J
    ...
    def inverse(self, x):
        ...
        # Define the inverse transformation T^{-1}(z')
        T_inv = lambda x: b * x + (1 - b) * ((x - t(b * x)) * torch.exp(-s(b * x)))

        # Correct log-determinant computation (negation of forward)
        log_det_J_inv = -torch.sum((1 - b) * s(b * x), dim=1)

        return T_inv(x), log_det_J_inv
...
if __name__ == "__main__":
    ...
    # python3 flow.py train --data tg --model model.pt --device mps --batch-size 10000 --epochs 1 --lr 1e-3
    ...
    num_transformations = 10 # Number of coupling layers: the more the more expressiveness
    ...
    if args.mode == 'train':
        ...
        n_data = 100000  # Increase data size for better density estimation
